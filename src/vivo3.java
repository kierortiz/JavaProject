...
public class vivo3 extends JFrame{
	JFrame frame;
	JPanel panel,panel1;
	...
	public vivo3(String usern) {
		
		this.usern=usern;
		frame = new JFrame("VIVO");
		panel = new JPanel();
	
		
		
		ImageIcon pic1 = new ImageIcon (new ImageIcon("vivos5.jpg").getImage().getScaledInstance(200, 250, Image.SCALE_DEFAULT));	
		sampic = new JLabel(pic1);
			sampic.setBounds(30,30,200,250);
			panel.add(sampic);
		desc = new JLabel("<html>Specs<br><br>"
				+ "Processor: Qualcomm Snapdragon 655 Plus<br><br>"
				+ "RAM: 4GB, 6GB Variant"
				+ "<br><br>Storage: 64GB, 128GB<br><br>"
				+ "Battery Capacity: 3000mAh<br><br>"
				+ "Rear Camera: 48MP, + 13MP <html>");
		...
		cmbColor = new JComboBox(cmb);
			...
			panel.add(cmbColor);
		p1 = new JLabel("22000 Pesos");
			p1.setBounds(250,330,100,20);
			panel.add(p1);
		p2 = new JLabel("26000 Pesos");
			...
			public void itemStateChanged(ItemEvent e)
			{
				...
				{	
					...
					if(rad1.isSelected())
					{
						...
						String logg1 = ("SELECT * FROM products WHERE prod_id='239'");
			...
			public void itemStateChanged(ItemEvent e)
			{
				...
				{	
					...
					if(rad2.isSelected())
					{
						...
						{
						st = cn.createStatement();
						String logg1 = ("SELECT * FROM products WHERE prod_id='240'");
						rs = st.executeQuery(logg1);
						if(rs.next())
						{
							prodid = rs.getInt("prod_id");
