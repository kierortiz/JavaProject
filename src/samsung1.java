...
public class samsung1 extends JFrame{
	JFrame frame;
	JPanel panel,panel1;
	...
	public samsung1(String usern) {
		...
		ImageIcon pic1 = new ImageIcon (new ImageIcon("sampic2.jpg").getImage().getScaledInstance(200, 250, Image.SCALE_DEFAULT));	
		sampic = new JLabel(pic1);
			sampic.setBounds(30,30,200,250);
			panel.add(sampic);
		desc = new JLabel("<html>Processor<br> 2.7 GHz, 2.3 GHz. 1.9GHz Octa-Core<br><br>"
				+ "Display<br>Dynamic AMOLED  3040 x 1440 (Quad HD+)<br><br>"
				+ "Camera<br>12MP + 16MP + 12MP Optical Zoom at 2x, Digital Zoom up to 10x"
				+ "UHD 4k(3840x2160) <br>@ 60 dps Video Recording Resolution<html>");			
		desc.setBounds(250,10,260,250);
			panel.add(desc);
		variant = new JLabel("VARIANT");
			variant.setBounds(30,300,100,20);
			panel.add(variant);
		rad1 = new JRadioButton("64GB Storage + 6GB RAM");
			rad1.setBounds(30,330,200,20);
			panel.add(rad1);
		rad2 = new JRadioButton("128GB Storage + 8GB RAM");
		...
		cmbColor = new JComboBox(cmb);
			...
			panel.add(cmbColor);
		p1 = new JLabel("35000 Pesos");
			p1.setBounds(250,330,100,20);
			panel.add(p1);
		p2 = new JLabel("49000 Pesos");
			...
			public void itemStateChanged(ItemEvent e)
			{
				...
				{	
					...
					if(rad1.isSelected())
					{
						...
						descrip = "64GB Storage + 6GB RAM";
						pricedb = 35000;
						try
						{
						st = cn.createStatement();
						String logg1 = ("SELECT * FROM products WHERE prod_id='213'");
			...
			public void itemStateChanged(ItemEvent e)
			{
				...
				{	
					...
					if(rad2.isSelected())
					{
						...
						{
						st = cn.createStatement();
						String logg1 = ("SELECT * FROM products WHERE prod_id='214'");
						rs = st.executeQuery(logg1);
						if(rs.next())
						{
							prodid = rs.getInt("prod_id");
